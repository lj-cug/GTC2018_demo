...
To run the co-processing enable coupled model, following files are required.

* Input files for both ocean and atmosphere model components
...
For the demo, a simple visualization pipeline for volume rendering of clouds is created. In this case, topography and coastline information is read from static files (coastline.vtk and topo.vtk) but relative humidity for 3d-atmospheric column (after vertical interpolation performed on the driver level) is provided by the atmospheric model component. 

The visualization pipeline and example output can be seen in following figures.

![vtk](figures/Fig01.png)

![vtk](figures/Fig02.png)

**Note:** For simplification, this example only demonstrate interaction from ATM to COP but ATM and OCN components still interact in both ways. The COP component also able to process data flowing from multiple model component (both ATM and OCN) and this is achieved by adding new fields and section to **exfield.tbl** configuration file (more information about the configuration file/s can be found in [here](https://github.com/uturuncoglu/RegESM/blob/master/docs/04_Usage.md)).
...
This will create a folder named **scripts/**. Also note that the visualization pipeline is created using **ParaView 5.4.1** under CENTOS operating system.

**Note:** User need to modify the paths of static files (for topography and coastline) in the co-processing plugin generated Python script (**state\_lr\_541\_v1\_clouds\_dvr.py**). So, search **coastlinevtk** and **topovtk** variables in the Python script and replace the paths with appropriate ones for your system.

## 4.4 Modifying Configuration Files

To run the model, user need to edit 
